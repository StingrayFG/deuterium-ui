...
export default function FileLinkBox({ isFailed, fileSize, fileUuid }) {
  ...
  const copyLink = async () => {
    navigator.clipboard.writeText('https://' + process.env.NEXT_PUBLIC_DOMAIN + '/' + fileUuid)
    setFade(true);
    await delay(2000);
    setFade(false);
  };
    ...
    return(
      <div onClick={copyLink} className='w-11/12 md:w-8/12 h-12 mx-auto mt-6 mb-8 grid 
        ...
        <p className={`text-center text-sky-200 font-sans text-xl md:text-2xl place-self-center 
          ...
          {process.env.NEXT_PUBLIC_DOMAIN + '/' + fileUuid}
        </p>
